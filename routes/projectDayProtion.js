...
router.get('/_project_day_protion_list',(req,res) => {
    ...
    printList(req,res,"_view_project_day_protion_list");
...
router.put('/_project_day_protion_list/:id',(req,res) => {
    ...
    const sqlQry = ("UPDATE _project_day_protion SET "+sqlUpdateRow+" WHERE portion_id="+req.params.id+";");
    db.query(sqlQry, (error, result) => {
        if(error){
            console.log(error);
            res.send(JSON.stringify(sendError(error.errno,error.sqlMessage)));
...
router.delete('/_project_day_protion_list/:id',(req,res) => {
    ...
    const sqlQry = "DELETE FROM _project_day_protion WHERE portion_id = "+req.params.id+" LIMIT 1;";
    db.query(sqlQry, (error, result) => {
        if(error){
            console.log(error);
            res.send(JSON.stringify(sendError(error.errno,error.sqlMessage)));
