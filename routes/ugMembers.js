...
router.get('/_config_ugmembers_list',(req,res) => {
    ...
    const sqlQry = "CALL get_config_ugmembers_list(\"("+qryStr+")\",\""+sortBy+"\",\""+order+"\","+start+","+end+");";
...
router.put('/_config_ugmembers_list/:id',(req,res) => {
    ...
    const sqlQry = "CALL update_config_ugmembers("+req.params.id+",\""+sqlUpdateRow+"\");";
    db.query(sqlQry, (error, result) => {
        if(error){
            console.log(error);
            res.send('Something went wrong. Please try again.');
...
router.delete('/_config_ugmembers_list/:id',(req,res) => {
    ...
    const sqlQry = "CALL delete_config_ugmembers("+req.params.id+");";
    db.query(sqlQry, (error, result) => {
        console.log(error);
    }); 
});
